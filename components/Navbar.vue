  ...
  >
    ...
    <div class="container justify-between items-center flex">
      <NuxtLink to="/">
        <NuxtPicture
          src="/logo.png"
          class="max-w-full md:max-w-[425px] min-w-[100px]"
          preload
          :alt="$t('alt.slawomir')"
          width="425"
          height="30"
      ...
      <div class="flex items-center gap-4 relative">
        ...
        >
          ...
          <span
            class="block w-5 h-0.5 bg-white transition-transform duration-300"
            :class="isNavMenuOpen ? 'translate-y-[6px] rotate-45' : ''"
          />
          <span
            class="block w-5 h-0.5 bg-white my-1 transition-opacity duration-300"
      ...
      <div
        v-if="isNavMenuOpen"
        id="mobile-nav"
        class="lg:hidden absolute left-0 right-0 top-[calc(100%+1rem)] z-10"
      >
        <div
          class="rounded-2xl border border-gray-700 bg-[#1a1a1a]/95 backdrop-blur p-4 flex flex-col gap-2 shadow-xl"
        >
          <NuxtLink
            v-for="link in navLinks"
            :key="`mobile-${link.to}`"
