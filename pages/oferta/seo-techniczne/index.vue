            ...
            <div>
              ...
              <ul class="space-y-4">
                <li v-for="i in 3" :key="i" class="flex items-start gap-3">
                  ...
                  <span>{{ t(`offer.technicalSeo.feature${i}`) }}</span>
                </li>
              </ul>
            </div>

            <div class="bg-black/20 rounded-2xl p-6 md:p-8 border border-white/10">
              <p class="text-4xl font-bold mb-6">{{ t('offer.priceOnRequest') }}</p>
              <p class="text-[var(--secondary-text-color)] mb-8">
                {{ t('offer.technicalSeo.subtitle') }}
              </p>

              <div class="flex flex-col sm:flex-row gap-3">
                <NuxtLink
                  :to="{ path: localePath('/contact'), query: { package: 'technicalSeo' } }"
...
import { TECHNICAL_SEO_FAQ_ITEMS } from '@/const/schemaOrg'

const { locale, t } = useI18n()
const localePath = useLocalePath()

const faqItems = TECHNICAL_SEO_FAQ_ITEMS

const relatedArticles = [
  { path: '/blog/google-search-console', title: 'blog.googleSearchConsole.title' },
...
watch(locale, () => {
  useSeoMeta({
    title: t('seo.technicalSeo.title'),
    description: t('seo.technicalSeo.description'),
    ogTitle: t('seo.technicalSeo.title'),
    ogDescription: t('seo.technicalSeo.description'),
    ogSiteName: t('seo.ogSiteName'),
    ogUrl: `https://slawomir-wozniak.pl${localePath('/oferta/seo-techniczne')}`,
